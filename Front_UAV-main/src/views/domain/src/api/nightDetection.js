...
export const nightDetectionApi = {
    ...
    async processImage(formData) {
        try {
            ...
            const response = await request.post('/night-detection/process', formData, {
                headers: {
    ...
    async processVideo(formData) {
        try {
            ...
            const response = await request.post('/night-detection/process-video', formData, {
                headers: {
                    'Content-Type': 'multipart/form-data'
                }
            })
    ...
    async getVideoProgress(processId) {
        try {
            ...
            const response = await request.get(`/night-detection/video-progress/${processId}`)
    ...
    async getHistory() {
        try {
            ...
            const response = await request.get('/night-detection/history')
    ...
    async checkVideoProgress(processId) {
        try {
            ...
            let response;
            let apiEndpoints = [
                `/night-detection/video-progress/${cleanProcessId}`,
                `/night-detection/video-status/${cleanProcessId}`,
                `/night-detection/task-status/${cleanProcessId}`
            ];
