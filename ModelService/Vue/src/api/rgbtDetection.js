...
export const rgbtDetectionApi = {
  ...
  async detectObjects(formData) {
    try {
      const response = await request.post('/api/rgbt-detection/detect', formData, {
        headers: {
  ...
  async processVideoPair(formData) {
    try {
      const response = await request.post('/api/rgbt-detection/process-video-pair', formData, {
        headers: {
          'Content-Type': 'multipart/form-data'
        }
      })
  ...
  async getHistory() {
    try {
      const response = await request.get('/api/rgbt-detection/history')
