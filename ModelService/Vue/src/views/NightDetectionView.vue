            ...
            <div class="result-images">
              <div class="image-comparison">
                ...
                <div class="enhanced-image">
                  ...
                  <div class="image-url" v-if="showDebugInfo">URL: {{ resultImageUrl }}</div>
                </div>
                <!-- 添加增强后图像（如果有） -->
                <div class="enhanced-image" v-if="detectionResult?.enhancedImageUrl">
                  ...
                  <div class="image-url" v-if="showDebugInfo">URL: {{ detectionResult.enhancedImageUrl }}</div>
                </div>
              </div>
            </div>
          </div>
...
const showDebugInfo = ref(true)  // 控制是否显示调试信息
...
const getFullImageUrl = (relativePath) => {
  ...
  // 如果是浏览器本地的 Blob URL，直接返回，避免被错误拼接成 /api/blob:... 导致 404
  ...
  if (relativePath.startsWith('http')) {
    ...
    // 静态资源路径，直接使用，不要添加/api前缀
    // 这里是关键修复，因为后端已经在main.py中配置了静态文件直接后缀下可访问
    fullUrl = relativePath
  } else if (relativePath.startsWith('/')) {
    // 其他带前导斜杠的路径
    fullUrl = `/api${relativePath}`
  } else {
    // 无前导斜杠的路径
    fullUrl = `/api/${relativePath}`
  }
...
const loadResultVideo = () => {
  ...
  // 根据经过多轮测试，发现后端实际生成的是.avi扩展名的AVI格式(MJPG编解码器)视频
  // 因此必须优先尝试.avi扩展名
  const port = BACKEND_PORT || 8081
  
  // 尝试多种可能的文件扩展名，按优先级降序排列
  const pathsToTry = [
    // 1. 首先尝试标准.avi扩展名 (当前后端实际输出的是这个)
    `http://localhost:${port}/static/night_detection/result_${processId.value}.avi?t=${Date.now()}`,
    // 2. 其次尝试.mp4.avi扩展名 (旧版后端可能输出)
    `http://localhost:${port}/static/night_detection/result_${processId.value}.mp4.avi?t=${Date.now()}`,
    // 3. 最后尝试标准.mp4扩展名 (包容旧版浏览器)
    `http://localhost:${port}/static/night_detection/result_${processId.value}.mp4?t=${Date.now()}`
  ]
  
  console.log('智能检测多种视频格式，首先尝试:', pathsToTry[0])
  
  // 递归尝试每个路径
  const tryNextPath = (index = 0) => {
    if (index >= pathsToTry.length) {
      console.error('所有视频格式尝试失败，无法加载视频')
      ElMessage.error('视频文件不存在或格式不支持')
      return
    }
    ...
    fetch(currentPath, { method: 'HEAD' })
      .then(response => {
        if (response.ok) {
          console.log(`找到可用视频文件 (#${index + 1}):`, currentPath)
          videoResultUrl.value = currentPath
          videoFileFound.value = true
          ElMessage.success(`成功加载视频（格式 #${index + 1}）`)
        } else {
          console.log(`视频格式 #${index + 1} 不存在，尝试下一个格式`)
          tryNextPath(index + 1)
        }
      })
      .catch(error => {
        console.error(`检查视频格式 #${index + 1} 出错:`, error)
        tryNextPath(index + 1)
      })
  }
