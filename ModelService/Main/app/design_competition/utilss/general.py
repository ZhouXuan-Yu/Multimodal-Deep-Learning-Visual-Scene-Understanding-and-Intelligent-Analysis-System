...
from app.design_competition.utilss.google_utils import gsutil_getsize
from app.design_competition.utilss.metrics import fitness
from app.design_competition.utilss.torch_utils import init_torch_seeds
...
def intersect(box_a, box_b):
    """ 计算两组边界框之间的交集区域
    Args:
        box_a (torch.Tensor): 形状为 [batch, A, 4] 的边界框坐标，格式为 [x1, y1, x2, y2]
        box_b (torch.Tensor): 形状为 [batch, B, 4] 的边界框坐标，格式为 [x1, y1, x2, y2]
    Returns:
        torch.Tensor: 形状为 [batch, A, B] 的交集区域
    """
    A = box_a.size(1)
    B = box_b.size(1)
    
    # 扩展边界框的坐标以进行成对比较
    max_xy = torch.min(box_a[:, :, 2:].unsqueeze(2).expand(box_a.size(0), A, B, 2),
                       box_b[:, :, 2:].unsqueeze(1).expand(box_b.size(0), A, B, 2))
    min_xy = torch.max(box_a[:, :, :2].unsqueeze(2).expand(box_a.size(0), A, B, 2),
                       box_b[:, :, :2].unsqueeze(1).expand(box_b.size(0), A, B, 2))
    ...
    # 计算交集区域的面积
    return inter[:, :, :, 0] * inter[:, :, :, 1]  # [batch, A, B]
