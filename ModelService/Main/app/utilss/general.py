...
def fitness(x):
    # 简化版fitness函数
    return (x[0] * 0.0 + x[1] * 0.0 + x[2] * 1.0 + x[3] * 0.5) / 2  # weighted combination

def check_img_size(img_size, s=32):
    """验证图像尺寸是strides的整数倍"""
    # 确保图像尺寸是所提供stride的整数倍
...
def make_divisible(x, divisor):
    """返回最接近的、可被divisor整除的x的值"""
    return math.ceil(x / divisor) * divisor

def non_max_suppression_face(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, labels=(), multi_label=False):
    """非极大值抑制 (NMS) 用于车牌检测结果的处理"""
    nc = prediction.shape[2] - 15  # 类别数量
    xc = prediction[..., 4] > conf_thres  # 置信度阈值
    
    # 设置参数
    min_wh, max_wh = 2, 4096  # (像素) 最小和最大框
    time_limit = 10.0  # 最大处理时间限制（秒）
    redundant = True  # 要求冗余检测
    multi_label &= nc > 1  # 多标签每框（需要nc > 1）
    merge = False  # 使用加权合并NMS
    
    t = time.time()
    output = [torch.zeros((0, 16), device=prediction.device)] * prediction.shape[0]
    for xi, x in enumerate(prediction):  # 每张图像的检测框
        # 应用置信度阈值
        x = x[xc[xi]]  # 置信度
        ...
        # 框（center_x, center_y, width, height）到框（x1, y1, x2, y2）
        box = xywh2xyxy(x[:, :4])
        
        # 检测
        i, j = (x[:, 15:] > conf_thres).nonzero(as_tuple=False).T
        x = torch.cat((box[i], x[i, j + 15, None], j[:, None].float(), x[i, 5:15]), 1)
...
def scale_coords(img1_shape, coords, img0_shape, ratio_pad=None):
    """将坐标缩放到原始图像大小"""
    if ratio_pad is None:  # 计算从 img1_shape 到 img0_shape 的比例
        gain = min(img1_shape[0] / img0_shape[0], img1_shape[1] / img0_shape[1])  # 增益 = 新 / 旧
        pad = (img1_shape[1] - img0_shape[1] * gain) / 2, (img1_shape[0] - img0_shape[0] * gain) / 2  # wh 填充
    else:
        gain = ratio_pad[0][0]
        pad = ratio_pad[1]
...
np.set_printoptions(linewidth=320, precision=5, suppress=True, formatter={'float_kind': '{:11.5g}'.format})  # format short g, %precision=5
