...
def init_model(device, model_path):
    ...
    # 打印权重的键值结构，便于调试
    print(f"权重文件键值: {list(model_state.keys())[:10]}{'...' if len(model_state.keys()) > 10 else ''}")
    ...
    try:
        ...
        print("成功加载myNet_ocr_color模型 (带颜色识别)")
    except Exception as e1:
        print(f"加载myNet_ocr_color模型失败: {str(e1)}")
        
        try:
            # 然后尝试不带颜色识别功能的模型
            model = myNet_ocr(num_classes=len(plateName), export=True, cfg=cfg)
            model.load_state_dict(model_state, strict=False)
            print("成功加载myNet_ocr模型 (不带颜色识别)")
        except Exception as e2:
            print(f"加载myNet_ocr模型也失败: {str(e2)}")
            
            # 最后的尝试，使用不带颜色识别的模型并禁用颜色识别
            model = myNet_ocr_color(num_classes=len(plateName), export=True, cfg=cfg, color_num=None)
            model.load_state_dict(model_state, strict=False)
            print("最后尝试成功: 加载myNet_ocr_color模型 (颜色识别禁用)")
            
    # 确保模型被加载成功
    if model is None:
        raise Exception("无法加载车牌识别模型")
