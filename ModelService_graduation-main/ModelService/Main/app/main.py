...
try:
    from app.routers import night_guardian
    modules['night_guardian'] = night_guardian
    print("成功加载 night_guardian 模块")
except ImportError as e:
    print(f"无法加载 night_guardian 模块: {str(e)}")
...
# 为夜间保卫者模块添加直接路由(类似于火灾检测的直接路由)
# 测试用简化处理路由
@app.get("/api/night-guardian/simple-test")
@app.get("/api/night_guardian/simple-test")
async def night_guardian_simple_test():
    """最简单的GET测试路由，不需要处理文件上传"""
    ...
    try:
        ...
        logger.exception(f"[简单测试路由] 错误: {str(e)}")
        return JSONResponse(
            status_code=500,
            content={
                "status": "error", 
...
# 测试用简化处理路由
@app.post("/api/night-guardian/test-video")
@app.post("/api/night_guardian/test-video")
async def night_guardian_test_video(request: Request):
    """简化的测试路由，只返回成功响应"""
    logger.info("[测试路由] 收到夜间保卫者视频测试请求")
    
    try:
        # 尝试解析表单数据
        form = await request.form()
        logger.info(f"[测试路由] 表单数据键: {list(form.keys())}")
        
        if "file" in form:
            file = form["file"]
            logger.info(f"[测试路由] 文件名: {file.filename}, 类型: {file.content_type}")
            content = await file.read()
            logger.info(f"[测试路由] 文件大小: {len(content)} 字节")
        ...
        logger.exception(f"[测试路由] 错误: {str(e)}")
        return JSONResponse(
            status_code=500,
            content={
                "status": "error", 
                "message": f"测试路由错误: {str(e)}",
                "timestamp": datetime.now().isoformat()
            }
        )

@app.post("/api/night-guardian/process-video")
@app.post("/api/night_guardian/process-video") # 同时支持两种命名格式
async def night_guardian_process_video_direct(request: Request, background_tasks: BackgroundTasks):
    """直接路由处理夜间保卫者视频上传和处理"""
    from app.services.night_guardian_service import NightGuardianService
    import traceback
    
    logger.info("[直接路由] 收到夜间保卫者视频处理请求")
    logger.info(f"[直接路由] 请求headers: {request.headers}")
    logger.info(f"[直接路由] 请求URL: {request.url}")
    
    try:
        # 尝试解析multipart表单数据
        form = await request.form()
        logger.info(f"[直接路由] 表单数据键: {list(form.keys())}")
        
        # 从表单中获取文件和参数
        if "file" not in form:
            logger.error("[直接路由] 未提供文件")
            return JSONResponse({"status": "error", "message": "未提供文件"}, status_code=400)
            
        file = form["file"]
        # 检查文件类型和属性
        logger.info(f"[直接路由] 文件对象类型: {type(file).__name__}")
        logger.info(f"[直接路由] 文件属性: 文件名={file.filename}, 内容类型={file.content_type}, 其他属性={dir(file)}")
        
        # 简化参数，使用固定的默认值
        model_type = form.get("model_type", "OptimizedActionNetLite")
        save_frames = form.get("save_frames", "False").lower() == "true"
        
        # 使用默认阈值和片段长度，不再从前端获取
        threshold = 0.6  # 固定默认阈值
        clip_len = 16    # 固定默认片段长度
        
        logger.info(f"[直接路由] 处理参数: 文件名={file.filename}, 模型={model_type}, 阈值={threshold}, 帧长={clip_len}, 保存帧={save_frames}")
        ...
        try:
            ...
            temp_file.close()
            logger.info(f"[直接路由] 创建了临时文件: {temp_file_path}")
            
            # 保存上传的文件
            try:
                content = await file.read()
                logger.info(f"[直接路由] 读取文件内容成功，大小: {len(content)} 字节")
                
                with open(temp_file_path, "wb") as f:
                    f.write(content)
                logger.info(f"[直接路由] 文件写入成功: {temp_file_path}")
            except Exception as read_error:
                logger.exception(f"[直接路由] 读取或写入文件失败: {str(read_error)}")
                return JSONResponse(
                    status_code=500,
                    content={"status": "error", "message": f"读取或写入文件失败: {str(read_error)}", "detail": str(read_error)}
                )
            
            # 生成任务ID
            task_id = f"{datetime.now().strftime('%Y%m%d%H%M%S')}_{os.path.splitext(file.filename)[0]}"
            logger.info(f"[直接路由] 生成任务ID: {task_id}")
            
            # 检查文件是否可读
            if not os.path.exists(temp_file_path):
                logger.error(f"[直接路由] 临时文件不存在: {temp_file_path}")
                return JSONResponse(
                    status_code=500,
                    content={"status": "error", "message": "临时文件创建失败"}
                )
            
            file_size = os.path.getsize(temp_file_path)
            logger.info(f"[直接路由] 临时文件大小: {file_size} 字节")
            
            if file_size == 0:
                logger.error(f"[直接路由] 临时文件大小为0: {temp_file_path}")
                return JSONResponse(
                    status_code=500,
                    content={"status": "error", "message": "上传的文件为空"}
                )
            
            # 创建服务实例
            try:
                service = NightGuardianService()
                logger.info("[直接路由] 创建服务实例成功")
            except Exception as service_error:
                logger.exception(f"[直接路由] 创建服务实例失败: {str(service_error)}")
                return JSONResponse(
                    status_code=500,
                    content={"status": "error", "message": f"创建服务实例失败: {str(service_error)}", "detail": str(service_error)}
                )
            ...
            try:
                ...
                logger.info(f"[直接路由] 成功添加后台任务，任务ID: {task_id}")
            except Exception as task_error:
                logger.exception(f"[直接路由] 添加后台任务失败: {str(task_error)}")
                return JSONResponse(
                    status_code=500,
                    content={"status": "error", "message": f"添加后台任务失败: {str(task_error)}", "detail": str(task_error)}
                )
            
            return {
                "task_id": task_id,
                "status": "processing",
                "message": "视频处理任务已启动，处理完成后将自动显示结果"
            }
        except Exception as inner_e:
            error_trace = traceback.format_exc()
            logger.exception(f"[直接路由] 处理文件时发生内部错误: {str(inner_e)}")
            logger.error(f"[直接路由] 错误堆栈跟踪: {error_trace}")
            return JSONResponse(
                status_code=500,
                content={
                    "status": "error", 
                    "message": f"处理文件时发生错误: {str(inner_e)}", 
                    "detail": str(inner_e),
        ...
        logger.exception(f"处理夜间保卫者视频时出错: {str(e)}")
        return JSONResponse(
            status_code=500,
            content={"status": "error", "message": f"处理视频时出错: {str(e)}"})
        ...
        # 保存到本地目录
        upload_dir = os.path.join(MAIN_DIR, "static", "uploads")
        ...
        # 将视频发送到Flask服务
        flask_url = "http://127.0.0.1:5000/video_upload"
        logger.info(f"转发视频到Flask服务: {flask_url}")
        
        try:
            with open(file_path, "rb") as file_data:
                # 尝试打印文件信息以便调试
                file_stats = os.stat(file_path)
                logger.info(f"文件大小: {file_stats.st_size} 字节")
                logger.info(f"MIME类型推测: video/mp4")
                
                # 使用多种字段名同时尝试上传
                # 这类似于夜视车辆检测系统中需要同时支持jpg和png的情况
                files = {
                    "video": (filename, file_data, "video/mp4"),
                    # 同时也使用"file"字段，以增加兼容性
                    "file": (filename, file_data, "video/mp4")
                }
                
                async with httpx.AsyncClient(timeout=60.0) as client:
                    # 添加详细的请求日志
                    logger.info(f"发送HTTP请求到Flask服务，headers: {{'Content-Type': 'multipart/form-data'}}")
                    logger.info(f"请求文件名: {filename}")
                    
                    try:
                        response = await client.post(flask_url, files=files)
                        logger.info(f"Flask服务响应状态码: {response.status_code}")
                        logger.info(f"Flask服务响应内容: {response.text[:200]}...")  # 只记录前200个字符
                        
                        if response.status_code == 200:
                            try:
                                result = response.json()
                                logger.info(f"解析JSON响应成功: {result}")
                                return JSONResponse({"success": True, "process_id": result.get('process_id', str(uuid.uuid4())), "message": "视频已上传，正在处理中"})
                            except Exception as json_e:
                                logger.error(f"解析JSON响应失败: {str(json_e)}")
                        else:
                            logger.error(f"Flask服务返回非200状态码: {response.status_code}")
                    except httpx.ReadTimeout:
                        logger.error("请求Flask服务超时")
                    except Exception as req_e:
                        logger.error(f"请求Flask服务时发生错误: {str(req_e)}")
        except Exception as e:
            logger.error(f"转发视频到Flask服务失败: {str(e)}")
        
        # 如果失败，返回模拟响应
        process_id = str(uuid.uuid4())
        logger.info(f"返回模拟处理ID: {process_id}")
        return JSONResponse({"success": True, "process_id": process_id, "message": "视频已上传，正在处理中(模拟)"})
    except Exception as e:
        logger.error(f"处理视频上传时出错: {str(e)}")
        return JSONResponse({"success": False, "message": f"处理视频时出错: {str(e)}", "process_id": str(uuid.uuid4())}, status_code=500)
...
try:
    ...
    # 注册使用短横线格式的路由(kebab-case)
    app.include_router(
        night_guardian.router,
        prefix="/api/night-guardian",
        tags=['夜间保卫者']
    )
    
    # 同时注册使用下划线格式的路由(snake_case)以保证兼容性
    app.include_router(
        night_guardian.router,
        prefix="/api/night_guardian",
        tags=['夜间保卫者']
    )
    
    # 详细记录路由信息以便调试
    for route in night_guardian.router.routes:
        logger.info(f'✅ 注册路由: [夜间保卫者] {route.path} 方法: {route.methods}')
    
    logger.info('✅ 成功注册夜间保卫者路由 (同时支持kebab-case和snake_case格式)')
except Exception as e:
    logger.error(f'❌ 注册夜间保卫者路由失败: {str(e)}')
