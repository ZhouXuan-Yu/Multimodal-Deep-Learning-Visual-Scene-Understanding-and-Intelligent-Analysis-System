<template>
  <div class="guide-system">
    ...
    <div class="quick-actions" v-if="showQuickActions">
      <el-button type="primary" @click="showSuggestions = !showSuggestions">
        <el-icon><QuestionFilled /></el-icon>
        <span>提供建议路线</span>
      </el-button>
    </div>
    ...
    >
      <div class="suggestions-content">
        <div v-for="(action, index) in quickActions" :key="index" class="suggestion-item">
          <el-card :body-style="{ padding: '16px' }" shadow="hover" @click="handleQuickAction(action)">
            <div class="suggestion-header">
              <el-icon class="suggestion-icon" :size="20"><component :is="action.icon" /></el-icon>
              <h3>{{ action.title }}</h3>
            </div>
            <p class="suggestion-desc">{{ action.description }}</p>
          </el-card>
        </div>
      </div>
    </el-dialog>
...
import { 
  ...
  QuestionFilled, Guide, TrendCharts, List, InfoFilled
} from '@element-plus/icons-vue'
import { ElDialog, ElButton, ElCard, ElIcon } from 'element-plus'
...
const quickActions = reactive([
  {
    id: 'route',
    title: '北京到上海路线',
    description: '规划一条从北京到上海的最佳路线',
    prompt: '帮我规划从北京到上海的路线',
    icon: 'Location'
  },
  {
    id: 'route',
    title: '广州到深圳路线',
    description: '规划一条从广州到深圳的最佳路线',
    prompt: '我想从广州到深圳，帮我规划路线',
    icon: 'Guide'
  },
  {
    id: 'route',
    title: '西安到成都自驾游',
    description: '规划一条从西安到成都的自驾游路线',
    prompt: '我计划从西安自驾到成都，请推荐路线',
    icon: 'Van'
  },
  {
    id: 'route',
    title: '多点路线规划',
    description: '规划一条经过多个景点的路线',
    prompt: '我想从{start}出发，经过{spots}，最后到达{end}的路线',
    icon: 'List'
  }
])
