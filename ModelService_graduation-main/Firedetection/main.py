...
if __name__ == "__main__":
    if Mode == 'Fire':
        ...
        # 从配置文件中获取断点续训相关参数
        resume = Config_classification.get('Resume', False)
        checkpoint_dir = Config_classification.get('CheckpointDir', None)
        print(f"\n{'-'*50}\n火灾检测训练模式\n{'-'*50}")
        print(f"断点续训: {'启用' if resume else '禁用'}")
        if resume:
            print(f"检查点目录: {checkpoint_dir if checkpoint_dir else '自动查找最新检查点'}")
        
        # 启动训练，支持断点续训
        train_keras(resume=resume, checkpoint_dir=checkpoint_dir)
        ...
        # 从配置文件中获取分割模型断点续训相关参数
        resume = config_segmentation.get('Resume', False)
        checkpoint_dir = config_segmentation.get('CheckpointDir', None)
        print(f"\n{'-'*50}\n火灾图像分割训练模式\n{'-'*50}")
        print(f"断点续训: {'启用' if resume else '禁用'}")
        if resume:
            print(f"检查点目录: {checkpoint_dir if checkpoint_dir else '自动查找最新检查点'}")
        ...
        # 调用分割模型训练函数，支持断点续训
        segmentation_keras_load(resume=resume, checkpoint_dir=checkpoint_dir)
